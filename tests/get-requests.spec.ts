...
test.describe("Automate individual GET edpoints", () => {
  test("Test 01- GET /pettypes", async ({ api }) => {
    const pettyTypesResponse = await api.path("/pettypes").getRequest(200);
    await expect(pettyTypesResponse).shouldMatchSchema(
      "pettyTypes",
      "getPettyTypes",
    );
  });
  ...
  test("Test 02- GET /vets", async ({ api }) => {
    const vetResponse = await api.path("/vets").getRequest(200);
  ...
  test("Test 03- GET /owners", async ({ api }) => {
    const ownersResponse = await api.path("/owners").getRequest(200);
  ...
  test("Test 04- GET /pets", async ({ api }) => {
    const petsResponse = await api.path("/pets").getRequest(200);
    await expect(petsResponse).shouldMatchSchema("pets", "getPets");
  });

  test("Test 05- GET /specialties", async ({ api }) => {
    const specialtiesResponse = await api.path("/specialties").getRequest(200);
    await expect(specialtiesResponse).shouldMatchSchema(
      "specialties",
      "getSpecialties",
    );
  ...
  test("Test 06- GET /visits", async ({ api }) => {
    const visitsResponse = await api.path("/visits").getRequest(200);
...
test.describe("Automate a sequence for GET endpoints", () => {
  test("Test 01- GET /pettypes/{petTypeId}", async ({ api }) => {
    ...
    const singlePettyTypeResponse = await api
      .path(`/pettypes/${id}`)
      .getRequest(200);
    await expect(singlePettyTypeResponse).shouldMatchSchema(
      "pettyTypes",
  ...
  test("Test 02- GET /vets/{vetId}", async ({ api }) => {
    const vetsResponse = await api.path("/vets").getRequest(200);
    await expect(vetsResponse).shouldMatchSchema("vets", "getVets");
    const id = vetsResponse[0].id;
    const singleVetResponse = await api.path(`/vets/${id}`).getRequest(200);
    await expect(singleVetResponse).shouldMatchSchema(
      "vets",
      "getVetsSingleObject",
    );
  ...
  test("Test 03- GET /owners/{ownerId}", async ({ api }) => {
    ...
    const singleOwnerResponse = await api.path(`/owners/${id}`).getRequest(200);
    await expect(singleOwnerResponse).shouldMatchSchema(
      "owners",
  ...
  test("Test 04- GET /pets/{petId}", async ({ api }) => {
    ...
    const singlePetResponse = await api.path(`/pets/${id}`).getRequest(200);
    await expect(singlePetResponse).shouldMatchSchema(
      "pets",
      "getPetsSingleObject",
    );
  ...
  test("Test 05- GET /specialties/{specialtyId}", async ({ api }) => {
    ...
    const singelSpecialtiesResponse = await api
      .path(`/specialties/${id}`)
      .getRequest(200);
    await expect(singelSpecialtiesResponse).shouldMatchSchema(
      "specialties",
  ...
  test("Test 06- GET /visits/{visitId}", async ({ api }) => {
    ...
    const singelVisitResponse = await api.path(`/visits/${id}`).getRequest(200);
    await expect(singelVisitResponse).shouldMatchSchema(
      "visits",
      "getVisitsSingleObject",
    );
