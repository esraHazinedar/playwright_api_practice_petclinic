...
test("TEST 02 - Update Veterinarian Details", async ({ api }) => {
  const getVets = await api.path("/vets").getRequest(200);
  ...
  const getSpecialties = await api.path("/specialties").getRequest(200);
  await expect(getSpecialties).shouldMatchSchema(
    "specialties",
    "getSpecialties",
  );
  const allAvailableSpecialtiesResponse = getSpecialties;
  const newSpecialty = allAvailableSpecialtiesResponse.find(
    (s) => !vetFirstSpecialty.some((v) => v.id === s.id),
  );
  const updatedVetPayLoad = { ...firstVet, specialties: [newSpecialty] };

  await api.path(`/vets/${vetId}`).body(updatedVetPayLoad).putRequest(204);
